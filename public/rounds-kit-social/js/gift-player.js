...
(function(scope){'use strict';
 ...
 function hasAlpha(video){try{const c=document.createElement('canvas');c.width=c.height=2;const g=c.getContext('2d');g.clearRect(0,0,2,2);g.drawImage(video,0,0,1,1,0,0,1,1);return g.getImageData(0,0,1,1).data[3]<16}catch(e){return false}}
 ...
 class Player{
  ...
  async preload(key){
   ...
   bundle.ready=Promise.all(Object.keys(LENGTH).map(phase=>new Promise(resolve=>{
    const v=document.createElement('video');bundle.videos[phase]=v;v.muted=true;v.defaultMuted=true;v.playsInline=true;v.preload='auto';v.loop=phase==='loop';v.setAttribute('playsinline','');v.setAttribute('muted','');v.setAttribute('aria-hidden','true');v.src=(scope.ROUNDS_MEDIA?.[key]?.[phase]||new URL(key+'/'+phase+'.webm',base).href);
   ...
   }))).then(ok=>{bundle.loading=false;bundle.ok=ok.every(Boolean)&&hasAlpha(bundle.videos.intro);if(this.destroyed)return bundle;if(!bundle.ok)this.root.dispatchEvent(new CustomEvent('rounds:asset-error',{bubbles:true,detail:{gift:key,reason:'Media unavailable; static poster used.'}}));this.trim(key);return bundle});
   return bundle.ready;
  }
 ...
 scope.RoundsGiftMedia={Player,timeline,keys:KEYS,phaseDurations:LENGTH,posterURL:key=>scope.ROUNDS_MEDIA?.[KEYS.includes(key)?key:'crown']?.poster||new URL((KEYS.includes(key)?key:'crown')+'/poster.png',base).href};
})(window);
